...
wire        src2_is_imm;    //src2选择信号，为1选择立即数
wire        res_from_mem;   //写回寄存器堆选择信号，为1选择从mem读出的数据
wire        gr_we;  //通用寄存器组写使能
wire        mem_we; //存储器写使能
wire        src_reg_is_rd;  //源寄存器是rd
wire [31:0] rj_value;   
wire [31:0] rkd_value;

wire [ 4:0] rf_raddr1;  //寄存器堆
wire [ 4:0] rf_raddr2;
wire [31:0] rf_wdata;
...
assign nextpc    = br_taken ? br_target : (pc+4'h4);//在这里实现nextpc信号的生成

assign imm      = {{20{i12[11]}},i12[11:0]};
assign alu_src1 = rj_value;
assign alu_src2 = src2_is_imm ? {{20{i12[11]}},i12} : rkd_value;//在这里实现alu_src2信号
